...
useEffect(() => {
  ...
  const fetchOptions = useCallback(debounce(async (searchKey) => {
    setLoading(true);
    try {
      const response = await apiKey({ languageId:languageId,type:iTypeF2,searchString: searchKey });
  ...
  const CustomListBox = React.forwardRef((props, ref) => {
    ...
    return (
      <ul style={{ paddingTop: 0 }} ref={ref} {...other}>
        <ListSubheader style={{ backgroundColor: useTheme.secondaryColor, padding: "5px" }}>
          <div
            style={{
              display: "flex",
